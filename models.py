...
class Ship(GImage):
    ...
    def __init__(self, x, y, width, height, source):
        """
        Initializes an instance of a Ship, using the GImage parent class.

        Parameter x: the x coordinate of the center of the Ship
        Precondition: x is a int 0<=x<=GAME_WIDTH

        Parameter y: the y coordinate of the center of the Ship
        Precondition: x is a int 0<=y<=GAME_HEIGHT

        Parameter width: the width of the Ship object in pixels
        Precondition: width is an int width>0

        Parameter height: the height of the Ship object in pixels
        Precondition: height is an int height>0

        Parameter source: the source file for the image of the Ship
        Precondition: source is a source file with extension .png
        """
    ...
    def collides(self, bolt):
        ...
        """
        boltlefttop = (bolt.x - BOLT_WIDTH // 2, bolt.y + BOLT_HEIGHT // 2)
        boltrighttop = (bolt.x + BOLT_WIDTH // 2, bolt.y + BOLT_HEIGHT // 2)
        boltleftbottom = (bolt.x - BOLT_WIDTH // 2, bolt.y - BOLT_HEIGHT // 2)
        boltrightbottom = (bolt.x + BOLT_WIDTH // 2, bolt.y - BOLT_HEIGHT // 2)
        if bolt.isPlayerBolt():
            return False
        elif self.contains(boltlefttop):
            return True
        elif self.contains(boltrighttop):
            return True
        elif self.contains(boltleftbottom):
            return True
        elif self.contains(boltrightbottom):
            return True
...
class Alien(GImage):
    ...
    def __init__(self, x, y, width, height ,source):
        """
        Initializes an instance of an Alien, using the GImage parent class.

        Parameter x: the x coordinate of the center of the Alien
        Precondition: x is a int 0<=x<=GAME_WIDTH - ALIEN_WIDTH // 2

        Parameter y: the y coordinate of the center of the Alien
        Precondition: y is a int 0<=y<=GAME_HEIGHT - ALIEN_HEIGHT // 2

        Parameter width: the width of the Alien object in pixels
        Precondition: width is an int width>0

        Parameter height: the height of the Alien object in pixels
        Precondition: height is an int height>0
    ...
    def collides(self, bolt):
        ...
        if bolt.isPlayerBolt() == False:
            return False
        elif self.contains(boltlefttop) == True :
            return True
        elif self.contains(boltrighttop) == True :
            return True
        elif self.contains(boltleftbottom) == True :
            return True
        elif self.contains(boltrightbottom) == True :
            return True
...
class Bolt(GRectangle):
    ...
    def __init__(self, x, y, width, height, fillcolor, linewidth, velocity):
        """
        Initializes an instance of a Bolt, using the GRectangle parent class.
        ...
        Parameter width: the width of the Bolt object in pixels
        Precondition: width is an int width>0

        Parameter height: the height of the Bolt object in pixels
        Precondition: height is an int height>0

        Parameter fillcolor: the color of the Bolt
        Precondition: fillcolor is a string representing a color e.g. 'blue'

        Parameter linewidth: the width of the exterior line of the Bolt
        Precondition: linewidth is an int linewidth>0

        Parameter velocity: the speed of the Bolt while it's on the screen
    ...
    def isPlayerBolt(self):
        """
        Returns True if self was fired from the Player, and returns False
        if self was fired from the ship.
        """
        if self._velocity > 0:
            return True
        if self._velocity < 0:
